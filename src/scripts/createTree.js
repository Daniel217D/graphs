...
const createTree = (depth) => {
    ...
    const levels = Array(depth).fill(0);

    const _createTree = (depth) => {
        if (depth-- === 0) {
            return undefined;
        }
        levels[depth] ++;
        return new Dot(powers2[start_depth - depth - 1] + levels[depth] - 1, _createTree(depth), _createTree(depth));
    };

    return _createTree(depth);
};
